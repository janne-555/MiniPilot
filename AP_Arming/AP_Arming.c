...
void AP_Arming_Update(void) {
   ...
   */
  arming.failsafe = AP_Failsafe_IsActive();

  /*
   * Battery status comes from failsafe decision
   */
  if (AP_Failsafe_GetReason() == FAILSAFE_BATTERY) {
    arming.battery_ok = 0;
  } else {
    arming.battery_ok = 1;
  }
   ...
   * Pre arm checks
   */
  if (arming.gyro_ok && arming.accel_ok && arming.rc_ok && arming.battery_ok &&
      !arming.failsafe) {
    arming.pre_arm_ok = 1;
  } else {
    arming.pre_arm_ok = 0;
  ...
  if ((arming.armed != last_armed) || (arming.pre_arm_ok != last_prearm) ||
    ...
    AP_Debug_Print(DBG_ARMING,
                   ...
                   "RC OK      : %u\n"
                   "Failsafe   : %u\n",
                   arming.armed, arming.pre_arm_ok, arming.gyro_ok,
                   arming.accel_ok, arming.compass_ok, arming.gps_ok,
                   arming.battery_ok, arming.rc_ok, arming.failsafe);
...
const AP_Arming_t *AP_Arming_Get(void) { return &arming; }
/*---------------------------------------------------------------------------
 * Status
 *--------------------------------------------------------------------------*/
