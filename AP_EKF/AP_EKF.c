...
void AP_EKF_Update(float dt) {
  ...
  ekf.roll = AP_AHRS_GetRoll();

  ekf.pitch = AP_AHRS_GetPitch();
  ...
  float gyro_yaw = AP_AHRS_GetYaw();

  float compass_yaw = AP_Compass_GetHeading();

  ekf.yaw = (gyro_yaw * EKF_YAW_GYRO_WEIGHT) +
  ...
  ekf.latitude = AP_GPS_GetLatitude();

  ekf.longitude = AP_GPS_GetLongitude();

  /*
   * BARO Fusion
   */

  ekf.altitude = AP_Baro_GetAltitude();

  ekf.velocity_z = AP_Baro_GetClimbRate();
  ...
  ekf.gps_ok = AP_GPS_IsHealthy();

  ekf.baro_ok = 1;

  ekf.compass_ok = AP_Compass_IsHealthy();

  /*
   * Overall EKF health
   */

  if (ekf.imu_ok && ekf.gps_ok && ekf.baro_ok && ekf.compass_ok) {
    ekf.healthy = 1;
  } else {
    ekf.healthy = 0;
  }
  ...
  if (debug_counter >= 100) {
    ...
    AP_Debug_Print(DBG_EKF,
                   ...
                   ekf.latitude, ekf.longitude,

                   ekf.altitude, ekf.velocity_z,

                   ekf.imu_ok, ekf.gps_ok, ekf.baro_ok, ekf.compass_ok,
