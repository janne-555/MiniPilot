...
void AP_FlightMode_Update(void) {
  ...
  if (AP_Failsafe_IsActive()) {
    ...
    case FAILSAFE_ACTION_RTL:
      ...
      AP_FlightMode_SetMode(FLIGHTMODE_RTL, MODE_REASON_FAILSAFE);

      break;

    case FAILSAFE_ACTION_LAND:

      AP_FlightMode_SetMode(FLIGHTMODE_LAND, MODE_REASON_FAILSAFE);

      break;

    case FAILSAFE_ACTION_HOLD:

      AP_FlightMode_SetMode(FLIGHTMODE_ALTHOLD, MODE_REASON_FAILSAFE);

      break;

    case FAILSAFE_ACTION_DISARM:

      AP_FlightMode_SetMode(FLIGHTMODE_FAILSAFE, MODE_REASON_FAILSAFE);
  ...
  case FLIGHTMODE_STABILIZE:
    ...
    flight_mode.desired_roll = rc->roll * MAX_ROLL_ANGLE;

    flight_mode.desired_pitch = rc->pitch * MAX_PITCH_ANGLE;

    flight_mode.desired_yaw = rc->yaw * MAX_YAW_RATE;

    flight_mode.desired_throttle = rc->throttle;
  ...
  case FLIGHTMODE_ACRO:
    ...
    flight_mode.desired_roll = rc->roll * MAX_ROLL_RATE;

    flight_mode.desired_pitch = rc->pitch * MAX_PITCH_RATE;
  ...
  case FLIGHTMODE_LAND:
    ...
    land_target = AP_InertialNav_GetAltitude() - 0.5f;

    if (land_target < 0.0f) {
      land_target = 0.0f;
    }

    AP_PosControl_SetAltTarget(land_target);
  ...
  case FLIGHTMODE_ALTHOLD:
    ...
    flight_mode.desired_throttle = AP_PosControl_GetThrottle();
  ...
  if (last_mode != flight_mode.mode) {
    AP_Debug_Print(DBG_FLIGHTMODE,
                   ...
                   flight_mode.mode, flight_mode.reason);
...
void AP_FlightMode_SetMode(AP_FlightMode_t mode,
  ...
  case FLIGHTMODE_ALTHOLD:
    ...
    AP_PosControl_SetAltTarget(AP_InertialNav_GetAltitude());
