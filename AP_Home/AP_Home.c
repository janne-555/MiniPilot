...
void AP_Home_Update(void) {
  ...
  if (home.valid == 0 && AP_EKF_IsHealthy()) {
    AP_Home_SetCurrent();
  }
  ...
  if (debug_counter >= 20) {
    ...
    AP_Debug_Print(DBG_HOME,
                   ...
                   home.valid, home.latitude, home.longitude, home.altitude,
                   AP_Home_GetDistance(), AP_Home_GetBearing());
  }
}
...
uint8_t AP_Home_SetCurrent(void) {
  ...
  home.latitude = AP_EKF_GetLatitude();

  home.longitude = AP_EKF_GetLongitude();

  home.altitude = AP_EKF_GetAltitude();

  home.heading = AP_EKF_GetYaw();
...
float AP_Home_GetDistance(void) {
  ...
  double lat1 = home.latitude * M_PI / 180.0;

  double lat2 = AP_EKF_GetLatitude() * M_PI / 180.0;

  double dlat = lat2 - lat1;

  double dlon = (AP_EKF_GetLongitude() - home.longitude) * M_PI / 180.0;
...
float AP_Home_GetBearing(void) {
  ...
  double lat1 = AP_EKF_GetLatitude() * M_PI / 180.0;

  double lat2 = home.latitude * M_PI / 180.0;

  double dlon = (home.longitude - AP_EKF_GetLongitude()) * M_PI / 180.0;
  ...
  float bearing = atan2(y, x) * 57.2957795f;
...
const AP_Home_t *AP_Home_Get(void) { return &home; }
