...
void AP_InertialNav_Update(float dt) {
  ...
  nav.altitude = AP_Baro_GetAltitude();

  /*
   * Vertical velocity
   */

  nav.velocity_z = (nav.altitude - last_altitude) / dt;

  last_altitude = nav.altitude;

  /*
   * GPS position
   */

  nav.latitude = AP_GPS_GetLatitude();

  nav.longitude = AP_GPS_GetLongitude();
