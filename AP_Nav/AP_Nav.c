...
void AP_Nav_Init(void) {
  ...
  AP_Nav_SetTarget(17.386000, 78.487000);
}
...
void AP_Nav_Update(void) {
  ...
  nav.current_lat = AP_InertialNav_GetLatitude();

  nav.current_lon = AP_InertialNav_GetLongitude();

  double lat1 = nav.current_lat * M_PI / 180.0;

  double lat2 = nav.target_lat * M_PI / 180.0;

  double dlat = (nav.target_lat - nav.current_lat) * M_PI / 180.0;

  double dlon = (nav.target_lon - nav.current_lon) * M_PI / 180.0;
  ...
  nav.bearing = atan2(y, x) * 180.0f / M_PI;

  /*
   * Simple position controller
   */

  nav.error_x = nav.distance * cos(nav.bearing);

  nav.error_y = nav.distance * sin(nav.bearing);

  nav.target_pitch = nav.error_x * 0.05f;

  nav.target_roll = nav.error_y * 0.05f;
  ...
  if (fabs(nav.distance - last_distance) > 1) {
    ...
    AP_Debug_Print(DBG_NAV,
                   ...
                   nav.distance, nav.bearing, nav.target_roll,
                   nav.target_pitch);

    last_distance = nav.distance;
  }
}
