...
void GCS_handle_mission_item(mavlink_message_t *msg) {
  ...
  printf("MISSION ITEM RX %u\n", item.seq);

  upload_seq++;

  if (upload_seq < upload_count) {

    GCS_send_mission_request(upload_seq);
...
void GCS_handle_mission_item_float(mavlink_message_t *msg) {
  ...
  printf("MISSION ITEM FLOAT RX %u\n", item.seq);
...
void GCS_handle_mission_request(mavlink_message_t *msg) {
  ...
  AP_Mission_Item_t *wp = AP_Mission_GetItem(req.seq);
  ...
  mavlink_msg_mission_item_int_pack(
      ...
      wp->lat * 10000000, wp->lon * 10000000, wp->alt,
