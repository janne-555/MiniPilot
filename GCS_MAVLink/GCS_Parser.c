...
static void handle_command_long(mavlink_message_t *msg) {
  ...
  case MAV_CMD_COMPONENT_ARM_DISARM:
    ...
    if (cmd.param1 == 1) {
      ...
      GCS_send_statustext("Vehicle Armed");
    } else {
      AP_Arming_Disarm();

      GCS_send_statustext("Vehicle Disarmed");
    }

    GCS_send_command_ack(cmd.command);
  ...
  case MAV_CMD_DO_SET_HOME:
    ...
    if (AP_Home_SetCurrent()) {
      GCS_send_statustext("Home position set");
      ...
      GCS_send_statustext("Home set failed");
    }
  ...
  case MAV_CMD_DO_SET_MODE: {
    ...
    AP_Mode_Set((AP_Mode_t)cmd.param2);
  ...
  case MAV_CMD_REQUEST_MESSAGE: {
    ...
    request_id = (uint16_t)cmd.param1;

    printf("REQUEST MESSAGE %u\n", request_id);

    GCS_handle_request_message(request_id);
  ...
  case MAV_CMD_SET_MESSAGE_INTERVAL: {
    ...
    msg_id = (uint32_t)cmd.param1;

    interval = (int32_t)cmd.param2;

    GCS_set_message_interval(msg_id, interval);
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (mavlink_parse_char(MAVLINK_COMM_0, rxbuf[i], &msg, &status)) {
      ...
      case MAVLINK_MSG_ID_SET_MODE: {
        ...
        AP_FlightMode_SetMode((AP_FlightMode_t)mode.custom_mode,
                              MODE_REASON_USER);
      ...
      case MAVLINK_MSG_ID_REQUEST_DATA_STREAM: {
        ...
        printf("REQUEST_DATA_STREAM\n"
               ...
               req.req_stream_id, req.req_message_rate, req.start_stop);

        GCS_stream_set_rate(req.req_stream_id, req.req_message_rate);
